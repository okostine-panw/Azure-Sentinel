...
LOG_TYPE = 'CortexCloudCases'

def main(mytimer: func.TimerRequest) -> None:
    """
    Azure Function to poll Cortex Cloud Cases API and send to Log Analytics
    ...
    try:
        # Get cases from Cortex Cloud
        cases = get_cortex_cloud_cases()
        
        if cases:
            logging.info(f'Retrieved {len(cases)} cases from Cortex Cloud')
            
            # Send to Log Analytics
            send_to_log_analytics(cases, LOG_TYPE)
            logging.info(f'Successfully sent {len(cases)} cases to Log Analytics')
        else:
            logging.info('No cases retrieved from Cortex Cloud')
...
def get_cortex_cloud_cases():
    """
    Call Cortex Cloud Cases API
    """
    url = f'https://api-{CORTEX_CLOUD_FQDN}/public_api/v1/case/search'
    ...
    try:
        ...
        for case in cases:
            transformed = {
                ...
                'CaseNumber': str(case.get('case_number', '')),
                'Title': str(case.get('title', '')),
                'Description': str(case.get('description', '')),
                'Priority': str(case.get('priority', '')),
                'Status': str(case.get('status', '')),
                'AssignedTo': str(case.get('assigned_to', '')),
                'RelatedIssues': case.get('related_issues', []),
                'CreatedTime': case.get('created_time', ''),
                'UpdatedTime': case.get('updated_time', ''),
                'ClosedTime': case.get('closed_time', ''),
                'Tags': case.get('tags', []),
                'RawData': json.dumps(case)
            }
