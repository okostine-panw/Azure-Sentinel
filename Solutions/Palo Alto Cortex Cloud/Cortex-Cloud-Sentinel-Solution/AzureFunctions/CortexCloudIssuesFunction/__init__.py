...
LOG_TYPE = 'CortexCloudIssues'

def main(mytimer: func.TimerRequest) -> None:
    """
    Azure Function to poll Cortex Cloud Issues API and send to Log Analytics
    ...
    try:
        # Get issues from Cortex Cloud
        issues = get_cortex_cloud_issues()
        
        if issues:
            logging.info(f'Retrieved {len(issues)} issues from Cortex Cloud')
            
            # Send to Log Analytics
            send_to_log_analytics(issues, LOG_TYPE)
            logging.info(f'Successfully sent {len(issues)} issues to Log Analytics')
        else:
            logging.info('No issues retrieved from Cortex Cloud')
...
def get_cortex_cloud_issues():
    """
    Call Cortex Cloud Issues API
    """
    url = f'https://api-{CORTEX_CLOUD_FQDN}/public_api/v1/issue/search'
    ...
    try:
        ...
        # Extract issues from reply.data
        ...
        for issue in issues:
            transformed = {
                ...
                'IssueId': str(issue.get('issue_id', '')),
                'Title': str(issue.get('title', '')),
                'Description': str(issue.get('description', '')),
                'Severity': str(issue.get('severity', '')),
                'Status': str(issue.get('status', '')),
                'Category': str(issue.get('category', '')),
                'AffectedAssets': issue.get('affected_assets', []),
                'CreatedTime': issue.get('created_time', ''),
                'ModifiedTime': issue.get('modified_time', ''),
                'Tags': issue.get('tags', []),
